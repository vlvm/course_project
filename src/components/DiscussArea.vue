...
export default {
  ...
  props: {
    AskCourseId: [String, Number],
    AskQuestionId: [String, Number],
    contentType: [String, Number],
    discussStuId: [String, Number],
    sourceFirstType: {
      default: () => 0,
    },
  },
  ...
  methods: {
    ...
    GetAskList() {
      ...
      setTimeout(() => {
        let data = {
          sourceFirstType: _this.sourceFirstType,
    ...
    ChoiceImg() {
      if (this.isStartRecorder) {
        this.$message.warning("正在录音，禁止发送");
        return;
      }
    ...
    Recording() {
      ...
      if (_this.audio.play) {
        _this.$message.warning("正在播放语音，禁止录音");
        return;
      }
    ...
    playSounds(item) {
      if (this.isStartRecorder) {
        this.$message.warning("正在录音，请先停止录音再播放！");
        return;
      }
      let _this = this;
