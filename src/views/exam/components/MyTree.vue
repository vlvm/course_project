<template>
  <div style="position: relative">
    <div
      ...
      :class="{
        noRoot: role === 2,
        recycle: $route.meta.moduleName === 'exam' && ![1, 3].includes(role),
        recycle2: $route.meta.moduleName === 'exam' && [1, 3].includes(role),
      }"
    >
      <CatalogueTree
        :redDotIds="redDotIds"
...
export default {
  ...
  methods: {
    getCatalogueList(role = 1) {
      getDirectory(role).then((res) => {
        this.catalogueList = res.extra;
        if (role === 2 && !this.catalogueList[0].childList) {
          this.catalogueList[0].childList = [];
        }
    ...
    catalogClick(item) {
      ...
      if (item.classify === 2 && item.creator !== this.creator) {
        this.$emit("changeTeacher", true);
      } else {
        this.$emit("changeTeacher", false);
      }
      this.$forceUpdate();
    ...
    addCatalogue() {
      ...
      createSubdirectory({
        ...
        let data = this.catalogueList;
        this.loop(data, id, (item) => {
          if (!item.childList) {
            item.childList = [];
            item.childList.push({ ...res.extra, childList: [] });
          } else {
            item.childList.push({ ...res.extra, childList: [] });
          }
        });
        this.catalogueList = Object.assign([], this.catalogueList, data);
        this.loop(this.catalogueList, id, (item) => {
