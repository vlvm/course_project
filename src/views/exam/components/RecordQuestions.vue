  ...
  >
    ...
    <template slot="footer">
      ...
      <template v-if="!isEditing">
        <a-button @click="addPaperQuestion(0)" :disabled="!confirmExit">
          确定并退出
        </a-button>
        <a-button
          type="primary"
          @click="addPaperQuestion(1)"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      attachmentMappingList: {
        // 附件id
        attachmentId: null,
        // 附件类型 0-试卷介绍；1-题干附件；2-解析附件；3-追加附件
        attachmentType: 1,
        questionId: _this.contentId,
        attachmentName: "",
        creator: "",
      },
      // 已经上传的文件列表
  ...
  methods: {
    ...
    addPaperQuestion(type) {
      ...
      if (this.contentId === "") {
        addPaperQuestion(params).then((res) => {
          this.$emit("changeContent", [...this.contentData, res.extra], true);
          this.fileList = [];
          this.$refs.quesAttachment.value = "";
          this.$refs.question.clear();
        ...
        if (this.contentData[index].contentType === 2) {
          ...
          if (mappingIdList.length > 0) {
            ...
            addPaperQuestion(params).then((res) => {
              let data = JSON.parse(JSON.stringify(this.contentData));
        ...
        addPaperQuestion(params).then((res) => {
          ...
          data.splice(index + this.questionNumber, 0, res.extra);
          this.$emit("changeContent", data, true);
          this.fileList = [];
          this.$refs.quesAttachment.value = "";
          this.$refs.question.clear();
    ...
    changePaperQuestion() {
      ...
      changePaperQuestion(params).then((res) => {
        const data = JSON.parse(JSON.stringify(this.contentData));
