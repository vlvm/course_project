<template>
  <div
    ...
    @click="questionClick"
  >
    <!--<ul v-show="contentId === data.id && isNoChecking">
          ...
          <template v-if="stuId === 0 && !isReply">
            <div @click.stop>
              <a-collapse :bordered="false" @change="seeStuAnswers">
                <a-collapse-panel key="1" :disabled="!isOverRecoveryTime">
                  <div slot="header">
...
export default {
  ...
  methods: {
    ...mapActions("p", ["changeContentCheckedArr", "changeRelationData"]),
    // 获取当前可以多提关联的试题
    checkRelation() {
    ...
    // 查看某道题学生作答
    seeStuAnswers() {
      getQuesStuAnswer(this.data.paperId, this.data.id, this.stuId).then(
        (res) => {
          this.stuAnswerData = res.extra;
        }
      );
      getAnswerShareData(this.data.paperId, this.data.id).then((res) => {
        this.stuShareAnswerData = res.extra;
      });
    },
    // 删除某个学生得分享作答
    delStuShareAnswer(id) {
      delAnswerShareData(this.data.paperId, id, this.data.id).then((res) => {
        this.stuShareAnswerData = res.extra;
      });
    },
    // 点击未批阅作答
