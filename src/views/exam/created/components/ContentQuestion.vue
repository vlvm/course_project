<template>
  <div
    class="quesContent"
    :class="{
      active: contentId === data.id && isNoChecking,
      important: data.pointSwitch === 1 || data.mustHandleSwitch === 1,
    }"
    @click="questionClick"
  >
    <ul v-show="contentId === data.id && isNoChecking && !isOtherTeacher">
      ...
      <template v-if="contentType === 2">
        <div class="itemContent">
          ...
          <template v-if="showAnswerArea !== 2">
            ...
            <template v-if="contentId === data.id && isNoChecking">
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      copyData: (state) => state.exam_create.copyData,
    }),
  },
  methods: {
    ...
    changeNumber() {
      ...
      setQuestionNumber([
        ...
        if (res.extra) {
          ...
          const i = this.contentData.findIndex(
            (item) => item.content.id === this.data.id
          );
          data[i].content.questionNumber = this.questionNumber;
    ...
    changeScore(val) {
      addQuestionStore([
        ...
        if (res.extra) {
          ...
          data[i].content.questionScore = val;
          this.$emit("changeContent", data);
        }
      });
    },
    // 获取当前可以多提关联的试题
    checkRelation() {
      console.log("relationData:", this.relationData);
    ...
    // 内容点击
    questionClick() {
      if (this.contentId === this.data.id || !this.isNoChecking) {
        this.$emit("update:contentId", "");
        return;
      }
      this.getSerialNumberData();
      this.checkRelation();
      this.$emit("update:contentId", this.data.id);
    },
    // 试题编辑菜单点击
    menuClick(type) {
      const _this = this;
      ...
      switch (type) {
        ...
        case -1:
          ...
          cancelQuestionAssociated(
            ...
            this.$emit("update:changeContent", data);
          });
          break;
        // 多题关联
        case "0":
          this.$emit("update:editorType", type);
          break;
        // 连续编号
        case "1":
          this.$emit("update:editorType", type);
        ...
        case "5":
          ...
          if (this.copyData.content.length === 0) {
            this.$message.warning("请先复制考试内容！");
            return;
          }
          // eslint-disable-next-line no-case-declarations
          let { content, originPaperId } = this.copyData;
          copyContent(content, this.paperId, originPaperId).then((res) => {
            const data = JSON.parse(JSON.stringify(this.contentData));
            const i = data.findIndex(
              (item) => item.content.id === this.data.id
            );
            data.splice(i + 1, 0, ...res.extra);
            this.$emit("changeContent", data, true);
          ...
          this.$confirm({
            ...
            onOk: () => {
              if (_this.contentType === 0) {
                deletePaperText([id], _this.paperId).then(() => {
                  _this.$emit(
                    "changeContent",
                    _this.contentData.filter((o) => o.content.id !== id)
                  );
                  _this.$emit("update:contentId", "");
                  _this.$emit("update:editorType", "");
                });
              } else if (_this.contentType === 2) {
                let mappingIdList = this.contentSelector.content.mappingIdList;
                // 判断是否是多题关联
                if (mappingIdList.length > 0) {
                  delPaperQuestion(mappingIdList).then(() => {
                    const data = _this.contentData.filter(
                      (o) => !mappingIdList.includes(o.content.id)
                    );
                    _this.$emit("changeContent", data);
                    _this.$emit("update:contentId", "");
                  });
                } else {
                  delPaperQuestion([id]).then(() => {
                    const data = _this.contentData.filter(
                      (o) => o.content.id !== id
                    );
                    _this.$emit("changeContent", data);
    ...
    relationChange(e, id) {
      ...
      this.$emit("update:contentCheckedArr", this.relationData.slice(0, i + 1));
    },
    // 连续编号选择试题
    numberChange(id) {
      /*let arr = this.contentData
        ...
        .filter((o) => o);
      this.$emit("update:contentCheckedArr",arr);*/
      let i = this.serialNumberData.findIndex((o) => o === id);
      this.$emit(
        "update:contentCheckedArr",
        this.serialNumberData.slice(0, i + 1)
      );
    },
    // 考试内容多选
    checkContent(e) {
      if (e.target.checked) {
        this.$emit("update:contentCheckedArr", [
          ...this.contentCheckedArr,
          this.data.id,
        ]);
      } else {
        let ids = this.contentCheckedArr.filter((id) => id !== this.data.id);
        this.$emit("update:contentCheckedArr", ids);
      }
    },
    getFileIcon(name) {
      switch (name) {
    ...
    changeAnswerType(type, id) {
      ...
      if (type === 0) {
        setAnswerWay({
          ...
          const data = this.contentData;
          data[index].content.answerWayPhoto =
        ...
        setAnswerWay({
          ...
          const data = this.contentData;
          data[index].content.answerWayHandle =
        ...
        setAnswerWay({
          ...
          const data = this.contentData;
          data[index].content.answerWayTalking =
        ...
        setAnswerWay({
          ...
          const data = this.contentData;
          data[index].content.answerWayVideo =
    ...
    changeContent(data, filter = false) {
      this.$emit("changeContent", data, filter);
    },
  },
};
</script>
